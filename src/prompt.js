...
export function buildUserPrompt(stockData, userInput) {
  ...
  const dataBlocks = [
    ...
    safeStringify(d.incomeStatements, 'INCOME STATEMENTS (Annual, last 4 years)'),
    safeStringify(d.incomeStatementsQuarterly, 'INCOME STATEMENTS (Quarterly, last 8 quarters)'),
    safeStringify(d.balanceSheet, 'BALANCE SHEET (Annual, last 4 years)'),
    safeStringify(d.balanceSheetQuarterly, 'BALANCE SHEET (Quarterly, last 4 quarters)'),
    safeStringify(d.cashFlow, 'CASH FLOW STATEMENT (Annual, last 4 years)'),
    safeStringify(d.cashFlowQuarterly, 'CASH FLOW STATEMENT (Quarterly, last 4 quarters)'),
    safeStringify(d.ratios, 'FINANCIAL RATIOS (Annual, last 4 years)'),
    safeStringify(d.ratiosQuarterly, 'FINANCIAL RATIOS (Quarterly)'),
    safeStringify(d.keyMetrics, 'KEY METRICS (Annual, last 4 years)'),
    safeStringify(d.keyMetricsQuarterly, 'KEY METRICS (Quarterly)'),
    safeStringify(d.enterpriseValue, 'ENTERPRISE VALUE'),
    safeStringify(d.estimates, 'ANALYST ESTIMATES (EPS & Revenue)'),
    safeStringify(d.analystRecs, 'ANALYST RECOMMENDATIONS (Buy/Hold/Sell consensus)'),
    safeStringify(d.grades, 'ANALYST GRADES (Recent upgrades/downgrades)'),
    safeStringify(d.institutionalHolders, 'TOP INSTITUTIONAL HOLDERS'),
    safeStringify(d.insiderTrading, 'INSIDER TRADING (Recent transactions)'),
    safeStringify(d.peers, 'PEER COMPANIES'),
    safeStringify(d.peerComparison, 'PEER COMPARISON DATA (Financials for each peer)'),
    safeStringify(d.historicalPrices, 'HISTORICAL DAILY PRICES (Last 12 months)'),
    safeStringify(d.rsi, 'RSI (14-day)'),
    safeStringify(d.sma50, 'SMA 50-DAY'),
    safeStringify(d.sma200, 'SMA 200-DAY'),
    safeStringify(d.fred, 'MACRO DATA (FRED — Fed Funds Rate, CPI, Unemployment, GDP, 10Y Yield, etc.)'),
  ].join('\n\n');
